class SpaceShip
  ...
  def initialize
    ...
    @rect_center = Rectangle.new(0, 0, 56, @height * @scale - 25)
    @rect_right = Rectangle.new(0, 0, 35, 70)
    @rect_left = Rectangle.new(0, 0, 35, 70)
  ...
  def update(boosters, monsters)
    ...
    if button_down?(KB_SPACE) then
      if @last_bullet == nil or @last_bullet.start_y - @last_bullet.y >= 60 or !@last_bullet.active then
        @bullets.each do |bullet|
          if !bullet.active then
            bullet.active = true
            bullet.x = @x + @width * @scale / 2 - 1
  ...
  def draw
    if @is_invicible and !@touched then
      ...
      @orb.draw(@x - 3, @y - 3, 500, @scale, @scale, Color.new(@orb_alpha, 255, 255, 255))
    end
