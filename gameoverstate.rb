class GameOverState
  ...
  def update
    ...
    if @defining_score then
      @final_score += 100 if @final_score < ($score + $distance).to_i
      @final_score = ($score + $distance).to_i if @final_score > ($score + $distance).to_i
      if @final_score == ($score + $distance).to_i then
        @defining_score = false
      end
    else
      @scale_time += 0.1
  ...
  def draw
    ...
    $font.draw("Game score : #{$score.to_i} meters",
               ($game.width - $font.text_width("Distance travelled : #{$distance.to_i} meters", 0.4)) / 2,
               250,
               500,
               0.4,
               0.5,
    ...
    $font.draw("Distance travelled : #{$distance.to_i} meters",
               ($game.width - $font.text_width("Distance travelled : #{$distance.to_i} meters", 0.4)) / 2,
               350,
               500,
               0.4,
